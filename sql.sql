...
JOIN 
    asignatura a ON p.id = a.id_profesor
LEFT JOIN 
    alumno_se_matricula_asignatura ama ON a.id = ama.id_asignatura
WHERE 
    ama.id_asignatura IS NULL;
...
SELECT 
    ce.anyo_inicio, 
    COUNT(DISTINCT ama.id_alumno) AS numero_alumnos
FROM 
    curso_escolar ce
JOIN 
